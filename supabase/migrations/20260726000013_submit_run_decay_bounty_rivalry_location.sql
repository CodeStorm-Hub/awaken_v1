-- Combined submit_run() rewrite wiring decay/bounty/rivalry/location
-- together (refined territory plan item 4, the last piece). Builds on the
-- 20260725031613 version — same anti-cheat/idempotency logic, only the
-- post-closed-loop capture branch changes:
--   * last_defended_at bumped on every own-territory create/merge (decay
--     reset — a captured/defended territory's clock restarts).
--   * territory_captures logged per rival actually taken from (drives
--     current_rival() and the weekly leaderboard window), plus one row
--     for unclaimed land taken (loser_id null) when applicable.
--   * bounty zone multiplier applied as a separate bonus_area_sqm in the
--     response only — never inflates the stored polygon/territories.area_sqm.
--   * profiles.last_run_location updated on every accepted run (closed or
--     not) — powers nearby_leaderboard().
...
begin
  ...
  -- from rival takings) — a rough estimate (captured area minus whatever
  -- was actually removed from rivals), good enough for weekly-leaderboard
  -- credit without needing exact polygon accounting for a secondary stat.
  ...
  -- Bounty multiplier: a flat bonus credited on top of the real captured
  -- area, never scaling the stored polygon (bounty_zones migration's own
  -- doc comment) — checked against the run's end point falling inside an
  -- active zone's radius.
