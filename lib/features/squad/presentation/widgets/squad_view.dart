...
class SquadView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SquadCubit, SquadState>(
      listenWhen: (previous, current) =>
          !previous.refreshFailed && current.refreshFailed,
      listener: (context, state) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Couldn't refresh your squad — showing the last known data.",
            ),
          ),
        );
        context.read<SquadCubit>().dismissRefreshFailure();
      },
      child: Scaffold(
